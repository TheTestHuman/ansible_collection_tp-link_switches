...
"""
TP-Link SG3452X Batch VLAN Configuration Module
...
short_description: Batch VLAN configuration on TP-Link SG3452X switches
description:
    ...
    for i, vlan in enumerate(vlans):
        ...
        for port_type in ['tagged_ports', 'untagged_ports']:
            if port_type in vlan:
                ...
                for port in ports:
                    if not isinstance(port, int) or port < 1 or port > 52:
                        module.fail_json(msg=f"VLAN {vlan_id}: Invalid port {port} in '{port_type}' (must be 1-52)")
...
'''
    ...
    # Generate port configuration commands (TP-Link SG3452X syntax)
    # SG3452X uses:
